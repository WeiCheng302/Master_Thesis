...
LocalizationFile = RootFolder + "Localization_Mode\\Localization_Mode_Raw\\Localization_Mode_Raw_VC_MC.txt"

Outfile = RootFolder + "Localization_Rotation_" + RotFile[len(RootFolder):len(RootFolder)+ 2]+"_" + LocalizationFile[-9:-4] + "_Report3_ABS.txt"
...
with open(LocalizationFile, newline='') as csvfile: 
	
	for row in csv.reader(csvfile, delimiter=' '):
		for i, element in enumerate(SLAMTimeList):
			if element == float(row[0]) :
				quat = [float(row[4]), float(row[5]), float(row[6]), float(row[7])]
				RotM = R.from_quat(quat).as_matrix()
				LocalizationRotList.append(Rot7Para.dot(RotM))
...
for i in range(len(SLAMTimeList)):
	...
	if Rot7Para[0, 0] == LocalizationRotList[i][0, 0]:
		#print(Rot7Para[0, 0])
		#print(LocalizationRotList[i][0, 0])
		continue
		
	RotM = SfMRotList[i]
	Rdiff = LocalizationRotList[i].dot(RotM)
	...
	SLAMTimeListToWrite.append(SLAMTimeList[i])
...
#	count += 1	
#	Rdiff = LocalizationRotList[count].dot(RotM)
#	
#	phi = math.asin(Rdiff[0, 2])
#	
#	try:
...
with open(Outfile, 'w', newline='') as csvfile3:
		...
		for i in range(len(OmegaDiffList)):
			ToWrite = [str(SLAMTimeListToWrite[i]) + ' ' + str('{:.6f}'.format(OmegaDiffList[i])) + ' ' + str('{:.6f}'.format(PhiDiffList[i])) + ' ' + str('{:.6f}'.format(KappaDiffList[i]))]
			writer.writerow(ToWrite)

print(' ')			
print("       " + LocalizationFile + " Successfully Saved !!!")